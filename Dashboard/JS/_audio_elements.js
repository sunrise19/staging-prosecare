...
var inputElement = document.getElementById("upload_files")
//Listeners
...
function playAudio(recorderAudioAsBlob) {
    ...
    reader.onload = (e) => {
        ...
        let fileName = 'PC_VN_'+(new Date().getTime())+'.wav'
        let file = new File([recorderAudioAsBlob], fileName,{type:BlobType, lastModified:new Date().getTime()}, 'utf-8');
        let container = new DataTransfer(); 
        container.items.add(file);
        
        document.querySelector('#upload_files').files = container.files
